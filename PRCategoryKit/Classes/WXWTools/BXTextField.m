...
- (void)setStrtttttt:(NSString *)strtttttt{
    _strtttttt = strtttttt;
}
- (void)awakeFromNib {
    [super awakeFromNib];
    _strtttttt = @".";
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    UIButton *doneButton = [[UIButton alloc] initWithFrame:CGRectMake(doneButtonX, doneButtonY-KStatusBottomBarHeight, doneButtonW, doneButtonH-11)];
    doneButton.titleLabel.font = [UIFont systemFontOfSize:27];
    [doneButton setTitle:_strtttttt forState:(UIControlStateNormal)];
...
- (void)doneButton:(UIButton *)doneButton{
    ...
    if (_isReduction) {
        if ( ([self.text isEqualToString:@""] &&![self.text containsString:@"-"]) && ![_strtttttt isEqualToString:@""]) {
            // 获得光标所在的位置
        ...
        if (![self.text isEqualToString:@""] && ![self.text containsString:@"-"] && ![_strtttttt isEqualToString:@""] && self.text.length!=13) {
            // 获得光标所在的位置
            NSUInteger insertIndex = [self selectedRange].location;
